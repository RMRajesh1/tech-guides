...
function toggleNav(){
 if(isMobile()){
  ...
  try{ localStorage.setItem('bmcp-nav-collapsed', collapsed ? '1' : '0'); }catch(e){}
 }
}
...
function applyTheme(theme, animate){
 ...
 /* Persist */
 try{ localStorage.setItem('bmcp-theme', t); }catch(e){}
}
...
function setOS(os){
 ...
 try{ localStorage.setItem('bmcp-os', os); }catch(e){}
...
function toggleIssue(issue){
 ...
 const done = ()=>{ issue.dataset.animating = '0'; };
...
const spy = new IntersectionObserver(entries=>{
 entries.forEach(e => { e.isIntersecting ? visSet.add(e.target) : visSet.delete(e.target); });
 if(!visSet.size) return;
 let winner = null, best = Infinity;
 visSet.forEach(el=>{
  const t = el.getBoundingClientRect().top;
  if(t >= 0 && t < best){ best = t; winner = el; }
 });
...
(function fetchGitHubStars(){
 ...
 .then(data => {
  if(data && data.stargazers_count != null){
   ...
   apply(fmt(stars));
   try{ localStorage.setItem(CACHE_KEY, JSON.stringify({stars, ts: Date.now()})); }catch(e){}
  } else {
   apply('17K+');
  }
 })
